...
def save_nii(plaque, data):
    ...
    nib.save(plaque_nii, os.path.join(dealt_plaque_dir, 'mask_pipeline_vessel_plaque_view.nii.gz'))            
...
def plaque2vessel(plaque, plaque_num, out_plaque_data, vessel_num, vessel_left_data, out_vessel_data, vessel_data):
    ...
    #coord_plaque = []
    #for j in plaque_index:
    #    coord_plaque.extend(np.argwhere(out_plaque_data==plaque_num[j]))
        ...
        if len(plaque2vessel_num) > len(vessel_num) or len(plaque2vessel_num) > 1:
            for i in range(1, len(plaque2vessel_num)):
                if plaque2vessel_volume[i] > 100:
                    '''
                    vessel_max_region = vessel_dilation(vessel_max_region)
                    #coord_plaque = np.argwhere(out_plaque_data==plaque_num[i-1])
                    print(len(coord_plaque))
                    for index in range(len(coord_plaque)):
                        print(index)
                        if vessel_max_region[coord_plaque[index][0]][coord_plaque[index][1]][coord_plaque[index][2]] == 1:
                    ...
                    dilationed_vessel_data = vessel_dilation(dilationed_vessel_data)
                    #print(dilationed_vessel_data.shape)
                    #print(out_plaque2vessel_data.shape)
                    #coord_vessel_dilation = np.argwhere(dilationed_vessel_data==1)
                    ...
                    save_nii(plaque, vessel_dilation_arr)                        
