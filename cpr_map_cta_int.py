...
def compute_dice(pred, gt): 
    print(np.unique(pred), len(pred[np.nonzero(pred)]))
    print(np.unique(gt), len(gt[np.nonzero(gt)]))
    pred[pred>0] = 1
    gt[gt>0] = 1
...
def cta2cpr(cpr_coords, plaque_cta, cpr_map_index, patient, series, mask_names, cpr_counts):
    ...
    for p in range(cpr_counts*20, cpr_counts*20+20):
        ...
        if len(np.unique(cpr_data)) > 1:
            np.save('/mnt/users/a.npy',cpr_data)
        '''
        ...
        for r in range(len(area_index)):
            ...
            for i in indes:
                ...
                if (int(cpr_coord[i[0],i[1]][0]), int(cpr_coord[i[0],i[1]][1]), int(cpr_coord[i[0],i[1]][2])) not in cor_axis:
                    ...
                    cta_data[int(cpr_coord[i[0],i[1]][0]), int(cpr_coord[i[0],i[1]][1]), int(cpr_coord[i[0],i[1]][2])] = cpr_mask[i[0],i[1]]
        print(len(cor_axis))
...
for patient in sorted(os.listdir(plaque_mask_gen_path)):
    ...
    for series in os.listdir(os.path.join(plaque_mask_gen_path, patient)):
        ...
        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), cta_series.flip, cta_data, 'mask_plaque_int.nii.gz')
