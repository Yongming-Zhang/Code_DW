...
def cpr2cta_map(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names):
    ...
    for cpr_coord, cpr_mask in zip([cpr_coords[ii] for ii in cpr_map_index], [cpr_mask_data[jj] for jj in cpr_map_index]):
        ...
        for i in cor_axis:
            _cta_data[i] = 1
        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), False, _cta_data, 'mask_plaque_mean_cpr.nii.gz')
        '''
            ...
            for co_index in range(len(all_cta_coord)):
                ...
                for target_k in target_point:
                    ...
                    if len(coord_point[target_k]) >= 8:
                        ...
                        dist_val = []
                        for ix in dist:
                            dist_list.append(coord_dic[coord_point[target_k][ix][0], coord_point[target_k][ix][1], coord_point[target_k][ix][2]])
                            dist_val.append(distances[0][ix])
                        if distances[0][0] == 0:
                            values = coord_dic[coord_point[target_k][0][0], coord_point[target_k][0][1], coord_point[target_k][0][2]]
                            coord_dic[target_k] = values
                            print('coordvalues', values)
                        else:
                            dist_sum = sum(dist_val)
                            dist_val = dist_val/(dist_sum)
                            dist_weight = softmax(np.array([dist_val]))
                            print(dist_weight)
                            values = 0
                            for index in range(len(dist_list)):
                                values += dist_list[index] * dist_weight[0][index]
                            coord_dic[target_k] = values
                            print('coordvalues', values)
                    else:
                        ...
                        target_expand_list = []
                        for x in range(max(0,target_expand_x-2), min(target_expand_x+2, cta_shape[::-1][0])):
                            for y in range(max(0,target_expand_y-2), min(target_expand_y+2,cta_shape[::-1][1])):
                                for z in range(max(0,target_expand_z-2), min(target_expand_z+2,cta_shape[::-1][2])):
                        ...
                        for ix in dist:
                            ...
                            dist_list.append(coord_dic[target_expand_list[ix][0], target_expand_list[ix][1], target_expand_list[ix][2]])
                            dist_val.append(distances[0][ix])
                        if distances[0][0] == 0:
                            values = coord_dic[coord_point[target_k][0][0], coord_point[target_k][0][1], coord_point[target_k][0][2]]
                            coord_dic[target_k] = values
                            ...
                            print(dist_weight)
                            dist_sum = sum(dist_val)
                            dist_val = dist_val/(dist_sum+1e-4)
                            dist_weight = softmax(np.array([dist_val]))
                            values = 0
                            for index in range(len(dist_list)):
                                values += dist_list[index] * dist_weight[0][index]   
...
for patient in sorted(os.listdir(plaque_mask_gen_path)):
    ...
    for series in os.listdir(os.path.join(plaque_mask_gen_path, patient)):
        ...
        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), cta_series.flip, cta_data, 'mask_plaque_meanvalue.nii.gz')
