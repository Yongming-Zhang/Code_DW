...
def save_nii(plaque, data):
    ...
    nib.save(plaque_nii, os.path.join(dealt_plaque_dir, 'mask_vessel_dealt.nii.gz'))            
        ...
        if len(plaque2vessel_num) > len(vessel_num) or len(plaque2vessel_num) > 1:
            for i in range(1, len(plaque2vessel_num)):
                if plaque2vessel_volume[i] > 100:
                    ...
                    vessel_new_data[first_point_coord[0]][first_point_coord[1]][first_point_coord[2]] = 2
                    ...
                    vessel_new_data[second_point_coord[0]][second_point_coord[1]][second_point_coord[2]] = 2
                    ...
                    vessel_new_data[z_min:z_max+1, y_min:y_max+1, x_min:x_max+1] = 2
                    print(vessel_new_data[vessel_new_data==2])
                    #vessel_new_data[1:100][1:10][1:10] = 2
...
def generate_point_line(first_point_coord, second_point_coord, vessel_new_data): 
...
for plaque in plaque_list:
    ...
    if int(plaque.split('/')[4]) > 0: #in plaque_datasets: #< 1020000:
        #continue
        print(plaque)
