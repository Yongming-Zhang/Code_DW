        ...
        for i in range(len(all_cpr_coord)):
            ...
            for j in range(len(all_cpr_coord[i])):
                ...
                if len(allcontours) > 2:
                    allcontours_copy = []
                    allcontours_centerline_copy = []
                    for c in range(len(allcontours)):
                        if len(allcontours[c]) > 5:
                            allcontours_copy.append(allcontours[c])
                            allcontours_centerline_copy.append(allcontours_centerline[c])
                    allcontours = allcontours_copy
                    allcontours_centerline = allcontours_centerline_copy
                '''
                ...
                if i==0 and j == 14:
                    ...
                    centerline_right.append(allcontours_centerline[0])
                    domain_right_counts.append(domain_counts)
                    all_axis_coord_left.append(allcontours[1])
                    id_left.append(all_ids[i][j])
                    centerline_left.append(allcontours_centerline[1])
                    domain_left_counts.append(domain_counts)
                    break
                if all_axis_coord_left == []:     
                    if len(allcontours) == 2:
                    ...
                    if len(allcontours) > 2:
                        ...
                        for p in range(2, len(allcontours)):
                            min_distances_left = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[-1]))
                            min_distances_right = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[-1]))
                            if np.mean(min_distances_left) <= np.mean(min_distances_right):
                ...
                elif all_axis_coord_right == []: 
                    if len(allcontours) == 2:
                        print('中心线使该区域分为', len(allcontours), '个连通域')
                        min_distances0 = pairwise_distances(np.array(allcontours[0]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[-1]))
                        min_distances1 = pairwise_distances(np.array(allcontours[1]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[-1]))
                        if np.mean(min_distances0) <= np.mean(min_distances1):
                            all_axis_coord_left.append(allcontours[0])
                            id_left.append(all_ids[i][j])
                            centerline_left.append(allcontours_centerline[0])
                            domain_left_counts.append(domain_counts)
                    ...
                    if len(allcontours) == 1:
                        print('中心线使该区域分为', len(allcontours), '个连通域')
                        min_distances = pairwise_distances(np.array(allcontours[0]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[-1]))
                        if (min_distances<3**0.5).any():
                            all_axis_coord_left.append(allcontours[0])
                            id_left.append(all_ids[i][j])
                            centerline_left.append(allcontours_centerline[0])
                            domain_left_counts.append(domain_counts)
                    ...
                    for k in range(len(allcontours)):
                        min_distances_left = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[-1]))
                        min_distances_right = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[-1]))
                        if np.mean(min_distances_left) <= np.mean(min_distances_right):
...
def cpr2cta_map_direct(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names, contour_files, cpr_centerlines):
    ...
    for cout in range(len(cpr_centerlines)):
        ...
        for i in range(cpr_counts*20, (cpr_counts+1)*20):
            ...
            print(len(cta_data[cta_data==1]))
            cta_data_copy = cta_data.copy()
            cta_data_list.append(cta_data_copy)
            #if i%20 == 4:
            #    break
        dices = cta2cpr(cpr_coords, cta_data, cpr_map_index, patient, series, mask_names, cpr_counts, cta_data_list)
...
name_list = ['1036604']#['1036602', '1036604', '1036607', '1036631', '1036632', '1036610', '1036619', '1036630', '1036615', '1036625', '1073332', '1036624'] #['1036604']
all_dices = []
...
print('final_dice', np.mean(all_dices))