...
def cpr2cta_map(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names):
    ...
    _cta_data = np.zeros(cta_shape[::-1], dtype='uint8')
    ...
    for cpr_coord, cpr_mask in zip([cpr_coords[ii] for ii in cpr_map_index], [cpr_mask_data[jj] for jj in cpr_map_index]):
        ...
        for r in range(len(area_index)):
            indes = area_index[r]
            for i in indes:
                print(cpr_coord[i[0],i[1]])
                if (int(cpr_coord[i[0],i[1]][0]), int(cpr_coord[i[0],i[1]][1]), int(cpr_coord[i[0],i[1]][2])) not in cor_axis:
                    cor_axis.append((int(cpr_coord[i[0],i[1]][0]), int(cpr_coord[i[0],i[1]][1]), int(cpr_coord[i[0],i[1]][2])))
        print(len(cor_axis))
        for i in cor_axis:
            _cta_data[i] = 1
        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), False, _cta_data, 'mask_plaque_grid_cpr.nii.gz')
        '''
        ...
        if counts == 0:
            for r in range(len(area_index)):
                ...
                cta_coord = []
                for i in range(max(0,xy_min[0]-1), min(xy_max[0]+2,cpr_h)):
                    for j in range(max(0,xy_min[1]-1), min(xy_max[1]+2,cpr_w)):
                        x, y, z = cpr_coord[i, j]
        ...
        if counts == 20:
            ...
            for co_index in range(len(all_cta_coord)):
                ...
                value = []
                for i in range(len(cta_coord)):
                    value.append(coord_dic[cta_coord[i][0], cta_coord[i][1], cta_coord[i][2]])
                ...
                for target_k in target_point:
                    target_k = target_k[0]
                    print(target_k)
                    cta_coord = np.array(cta_coord)
                    
                    grid = GridData3D(cta_coord[:,0], cta_coord[:,1], cta_coord[:,2], value)
                    coord_dic[target_k] = grid(target_k[0], target_k[1], target_k[2])[0] 
                    print(coord_dic[target_k])
                    cta_data[target_k] = coord_dic[target_k]
                '''

                #矩阵计算
                target = []
                for target_k in target_point:
                    target.append(target_k[0])
                '''
                xyz_min = np.min(target, axis=0)
                xyz_max = np.max(target, axis=0)  
                print(xyz_min, xyz_max)
                grid_point = []
                for i in range(xyz_min[0], xyz_max[0]+1):
                    for j in range(xyz_min[1], xyz_max[1]+1):
                        for k in range(xyz_min[2], xyz_max[2]+1):
                            grid_point.append((i,j,k))
                '''
                cta_coord = np.array(cta_coord)
                grid_point = np.array(target)
                grid_x, grid_y, grid_z = grid_point[:,0], grid_point[:,1], grid_point[:,2]
                grid = GridData3D(cta_coord[:,0], cta_coord[:,1], cta_coord[:,2], value)
                v = grid(grid_x, grid_y, grid_z)
                cta_data[grid_x, grid_y, grid_z] = v
                cta_cpr_data[grid_x, grid_y, grid_z] = v
            
            #print(len(cta_data[np.nonzero(cta_data)]))
            #save_nii(os.path.join(plaque_mask_newmap_path, patient, series), False, cta_data, 'mask_plaque_grid_cpr.nii.gz')
