        ...
        for i in range(len(all_cpr_coord)):
            ...
            for j in range(len(all_cpr_coord[i])):
                ...
                for centerline in all_centerlines[i][j]: 
                    if 0 <= int(round(centerline[0])) <= xy_max[0] and 0 <= int(round(centerline[1])) <= xy_max[1]:
                ...
                if j == 0 or all_axis_coord_left == []: 
                    if len(allcontours) == 2:
                        ...
                        #all_axis_coord_left_first = all_axis_coord_left[0]
                        #all_axis_coord_right_first = all_axis_coord_right[0]
                    elif len(allcontours) == 1:
                        print('中心线没有使该区域分为两个连通域1', len(allcontours))
                        all_axis_coord_left.append(allcontours[0])
                        id_left.append(all_ids[i][j])
                        centerline_left.append(allcontours_centerline[0])
                        #all_axis_coord_left_first = all_axis_coord_left[0]
                    ...
                    if len(allcontours) == 2:
                        ...
                        print('中心线没有使该区域分为两个连通域2', len(allcontours))
            ...
            for le in id_left_copy:
                if id_left.count(le) > 1:
                    left_counts[le] = id_left.count(le)
            print(left_counts)
            for left_i in range(len(all_axis_coord_left)):
                if left_i < len(all_axis_coord_left)-1:  
                    #判断如果有两个相同编号的斑块连通域，则判断这两个的下一个，若有下一个挨着的编号，则分别与下一个连接，
                    #若没有，则要判断该区域是否被中心线分割，若有，则与中心线连接，若没有，则不连接，保存这些点。
                    if id_left[left_i] == id_left[left_i+1] and left_i+2 <= len(all_axis_coord_left)-1: 
                        if id_left[left_i+1] != id_left[left_i+2] and id_left[left_i+1]+1 == id_left[left_i+2]:
                            generate_point_lines(all_axis_coord_left[left_i], all_axis_coord_left[left_i+2], cta_data)
                            generate_point_lines(all_axis_coord_left[left_i+1], all_axis_coord_left[left_i+2], cta_data)
                    #if id_left[left_i] == id_left[left_i+1] and left_i+2 > len(all_axis_coord_left)-1: 
                    #    if centerline_left[left_i][0] != []:
                    #        generate_point_lines(all_axis_coord_left[left_i], centerline_left[left_i][0], cta_data)
                    if id_left[left_i]+1 == id_left[left_i+1]:
                        generate_point_lines(all_axis_coord_left[left_i], all_axis_coord_left[left_i+1], cta_data)
                    #判断当前所连接连通域的编号是否连续，若连续，并且最后的编号是19，开始编号是0，则19连接0
                    if len(id_left) >= 20:
                        left_list = list(set(id_left))
                        if left_list == 20:
                            generate_point_lines(all_axis_coord_left[-1], all_axis_coord_left[0], cta_data)
            
            for right_i in range(len(all_axis_coord_right)):
                if right_i < len(all_axis_coord_right)-1:  
                    #判断如果有两个相同编号的斑块连通域，则判断这两个的下一个，若有下一个挨着的编号，则分别与下一个连接，
                    #若没有，则要判断该区域是否被中心线分割，若有，则与中心线连接，若没有，则不连接，保存这些点。
                    if id_right[right_i] == id_right[right_i+1] and right_i+2 <= len(all_axis_coord_right)-1: 
                        if id_right[right_i] != id_right[right_i+1] and id_right[right_i]+1 == id_right[right_i+2]:
                            generate_point_lines(all_axis_coord_right[right_i], all_axis_coord_right[right_i+2], cta_data)
                            generate_point_lines(all_axis_coord_right[right_i+1], all_axis_coord_right[right_i+2], cta_data)
                    #if id_right[right_i] == id_right[right_i+1] and right_i+2 > len(all_axis_coord_right)-1: 
                    #    if centerline_right[right_i][0] != []:
                    #        generate_point_lines(all_axis_coord_right[right_i], centerline_right[right_i][0], cta_data)
                    if id_right[right_i]+1 == id_right[right_i+1]:
                        generate_point_lines(all_axis_coord_right[right_i], all_axis_coord_right[right_i+1], cta_data)
                    #判断当前所连接连通域的编号是否连续，若连续，并且最后的编号是19，开始编号是0，则19连接0
                    if len(id_right) >= 20:
                        right_list = list(set(id_right))
                        if right_list == 20:
                            generate_point_lines(all_axis_coord_right[-1], all_axis_coord_right[0], cta_data)

            '''
            if len(all_axis_coord_left) > 1:
                if id_left[-1] - id_left[-2] == 1:
                    generate_point_lines(all_axis_coord_left[0], all_axis_coord_left[1], cta_data)
                    all_axis_coord_left = all_axis_coord_left[1:]
                if len(id_left) >= 20:
                    if id_left[0] == 0 and id_left[-1] == 19:
                        generate_point_lines(all_axis_coord_left[-1], all_axis_coord_left_first, cta_data)
                        print('zhixingle')

                if id_left[-1] - id_left[-2] > 1:
                    generate_point_lines(all_axis_coord_left[0], all_axis_coord_left[1], cta_data)
                    all_axis_coord_left = all_axis_coord_left[1:]
            if len(all_axis_coord_right) > 1:
                if id_right[-1] - id_right[-2] == 1: 
                    generate_point_lines(all_axis_coord_right[0], all_axis_coord_right[1], cta_data)
                    all_axis_coord_right = all_axis_coord_right[1:]
                if len(id_right) >= 20:
                    if id_right[0] == 0 and id_right[-1] == 19:
                        generate_point_lines(all_axis_coord_right[-1], all_axis_coord_right_first, cta_data)
