...
def cta2cpr(cpr_coords, plaque_cta, cpr_map_index, patient, series, mask_names, cpr_counts):
    ...
    dice_value = []
    for p in range(cpr_counts*60, (cpr_counts+1)*60):
        ...
        '''
        cpr_h, cpr_w = cpr_coord.shape[:2]
        plaque_cta_data = np.zeros(plaque_cta.shape, dtype='uint8')
        coord = []
        for i in range(cpr_h):
            for j in range(cpr_w):
                if 0 <= cpr_coord[i, j][0] < cta_w and 0 <= cpr_coord[i, j][1] < cta_h and 0 <= cpr_coord[i, j][2] < cta_d:
                    coord.append(cpr_coord[i, j])
        ...
        plaque_cta_data[max(0,int(round(xyz_min[0]))):min(int(round(xyz_max[0])),cta_w), max(0,int(round(xyz_min[1]))):min(int(round(xyz_max[1])),cta_h), max(0,int(round(xyz_min[2]))):min(int(round(xyz_max[2])),cta_d)] = plaque_cta[max(0,int(round(xyz_min[0]))):min(int(round(xyz_max[0])),cta_w), max(0,int(round(xyz_min[1]))):min(int(round(xyz_max[1])),cta_h), max(0,int(round(xyz_min[2]))):min(int(round(xyz_max[2])),cta_d)]
        print(plaque_cta_data.shape)
        plaque_cta_data = np.zeros(plaque_cta.shape, dtype='uint8')
        #plaque_cta_data[100:200,100:200,100:200] = 1'''
        
        xs, ys, zs = cpr_coord[:, :, 0], cpr_coord[:, :, 1], cpr_coord[:, :, 2]
        #plaque_cta_data = cta_data_list[p%20]
        print(len(plaque_cta[plaque_cta==1]))
...
def cpr2cta_map(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names, contour_files, cpr_centerlines):
    ...
    for cout in range(len(cpr_centerlines)):
        ...
        for i in range(len(all_cpr_coord)):
            ...
            domain_left_counts = [] #存左连通域数量
            centerline_right = [] #存右边中心线
            domain_right_counts = [] #存右连通域数量
            for j in range(len(all_cpr_coord[i])):
                ...
                #save_nii(os.path.join(plaque_mask_newmap_path, patient, series), True, cpr_data, 'cpr_plaque_%s.nii.gz'%j)
                
                #用来保存mask的轮廓点
                allcontours = []
                #用来保存中心线
                allcontours_centerline = []
                #domain_counts存中心线把cpr划分的连通域数量
                domain_counts = len(area_cpr_data_index) 
                ...
                if j == 0 or all_axis_coord_left == []: 
                    ...
                    if len(allcontours) > 2:
                        ...
                        centerline_right.append(allcontours_centerline[1])
                        domain_right_counts.append(domain_counts)
                        for p in range(2, len(allcontours)):
                            min_distances_left = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[0]))
                            for d in range(len(all_axis_coord_left)):
                                distances_left = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[d]))
                                if np.min(distances_left) < np.min(min_distances_left):
                                    min_distances_left = distances_left
                            min_distances_right = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[0]))
                            for d in range(len(all_axis_coord_right)):
                                distances_right = pairwise_distances(np.array(allcontours[p]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[d]))
                                if np.min(distances_right) < np.min(min_distances_right):
                                    min_distances_right = distances_right
                            if np.min(min_distances_left) <= np.min(min_distances_right):
                                all_axis_coord_left.append(allcontours[p])
                                id_left.append(all_ids[i][j])
                                centerline_left.append(allcontours_centerline[p])
                                domain_left_counts.append(domain_counts)
                            else:
                                all_axis_coord_right.append(allcontours[p])
                                id_right.append(all_ids[i][j])
                                centerline_right.append(allcontours_centerline[p])
                                domain_right_counts.append(domain_counts)
                    ...
                    for k in range(len(allcontours)):
                        if len(all_axis_coord_right) != 0:
                            min_distances_left = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[0]))
                            for d in range(len(all_axis_coord_left)):
                                distances_left = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_left[d]))
                                if np.min(distances_left) < np.min(min_distances_left):
                                    min_distances_left = distances_left
                            min_distances_right = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[0]))
                            for d in range(len(all_axis_coord_right)):
                                distances_right = pairwise_distances(np.array(allcontours[k]), device=torch.device("cuda"), y=np.array(all_axis_coord_right[d]))
                                if np.min(distances_right) < np.min(min_distances_right):
                                    min_distances_right = distances_right
                            if np.min(min_distances_left) <= np.min(min_distances_right):
                            ...
                            for d in range(len(all_axis_coord_left)):
                                ...
                                if np.min(distances_left) < np.min(min_distances_left):
                                    min_distances_left = distances_left
                            if (min_distances_left<3**0.5).any():
                                all_axis_coord_left.append(allcontours[k])
                                id_left.append(all_ids[i][j])
                                centerline_left.append(allcontours_centerline[k])
                                domain_left_counts.append(domain_counts)
                                ...
                                centerline_right.append(allcontours_centerline[k])
                                domain_right_counts.append(domain_counts)
                    print('all_axis_coord_left1', len(all_axis_coord_left)) 
                    print('all_axis_coord_right1', len(all_axis_coord_right)) 
                    ...
                    if id_left[left_i]+1 == id_left[left_next]:
                        ...
                        for left_in in range(left_i, left_next):
                            ...
                            for left_jn in range(left_next, left_next_next):
                                #计算left_in与left_jn的距离，找到left_in与left_jn最近距离对应的索引
                            ...
                            generate_point_lines(all_axis_coord_left[left_in], all_axis_coord_left[left_next+ind], cta_cpr_data)
                        ...
                        for left_in in range(left_next, left_next_next):
                            ...
                            generate_point_lines(all_axis_coord_left[left_in], all_axis_coord_left[left_i+ind], cta_cpr_data)
                        ...
                        while left_i < left_next:
                            if centerline_left[left_i][0] == []:
                                ...
                                generate_data(all_axis_coord_left[left_i], cta_cpr_data)
                            elif centerline_left[left_i][0] != [] and domain_left_counts[left_i] == 2:
                                ...
                                generate_point_lines(all_axis_coord_left[left_i], centerline_left[left_i][0], cta_cpr_data)
                            left_i += 1
                    left_i = left_next
                elif left_i == len(id_left)-left_counts[id_left[left_i]]:  
                    temp = left_i
                    while left_i < temp+left_counts[id_left[temp]]:
                        if centerline_left[left_i][0] == []:
                            #将这些值赋值到cta_data
                            generate_data(all_axis_coord_left[left_i], cta_data)
                            generate_data(all_axis_coord_left[left_i], cta_cpr_data)
            ...
            if len(id_left) >= 20:
                ...
                if left_list == 20:
                    ...
                    for left_in in range(left_ind, left_ind_next):
                        ...
                        generate_point_lines(all_axis_coord_left[left_in], all_axis_coord_left[left_0+ind], cta_cpr_data)
                    ...
                    for left_in in range(left_0, left_0_next):
                        ...
                        generate_point_lines(all_axis_coord_left[left_in], all_axis_coord_left[left_ind+ind], cta_cpr_data)
                ...
                if right_i < len(id_right)-right_counts[id_right[-1]]:  
                    ...
                    if id_right[right_i]+1 == id_right[right_next]:
                        ...
                        for right_in in range(right_i, right_next):
                            ...
                            generate_point_lines(all_axis_coord_right[right_in], all_axis_coord_right[right_next+ind], cta_cpr_data)
                        ...
                        for right_in in range(right_next, right_next_next):
                            ...
                            generate_point_lines(all_axis_coord_right[right_in], all_axis_coord_right[right_i+ind], cta_cpr_data)
                        ...
                        while right_i < right_next:
                            if centerline_right[right_i][0] == []:
                                ...
                                generate_data(all_axis_coord_right[right_i], cta_cpr_data)
                            elif centerline_right[right_i][0] != [] and domain_right_counts[right_i] == 2:
                                ...
                                generate_point_lines(all_axis_coord_right[right_i], centerline_right[right_i][0], cta_cpr_data)
                            right_i += 1
                    right_i = right_next
                elif right_i == len(id_right)-right_counts[id_right[right_i]]:  
                    temp = right_i
                    while right_i < temp+right_counts[id_right[temp]]:
                        if centerline_right[right_i][0] == []:
                            #将这些值赋值到cta_data
                            generate_data(all_axis_coord_right[right_i], cta_data)
                            generate_data(all_axis_coord_right[right_i], cta_cpr_data)
            ...
            if len(id_right) >= 20:
                ...
                if right_list == 20:
                    ...
                    for right_in in range(right_ind, right_ind_next):
                        ...
                        generate_point_lines(all_axis_coord_right[right_in], all_axis_coord_right[right_0+ind], cta_cpr_data)
                    ...
                    for right_in in range(right_0, right_0_next):
                        ...
                        generate_point_lines(all_axis_coord_right[right_in], all_axis_coord_right[right_ind+ind], cta_cpr_data)

        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), True, cta_data, 'mask_plaque_%s.nii.gz'%cpr_counts)
        
        cta_cpr_data = fill_holes(cta_cpr_data, 0, to_numpy=True)
        cta_cpr_data = (cta_cpr_data>=128).astype(np.float32)
        dices = cta2cpr(cpr_coords, cta_cpr_data, cpr_map_index, patient, series, mask_names, cpr_counts)
    ...
    print('all_dices', np.mean(all_dices))
    #cta_data = fill_holes(cta_data, 0, to_numpy=True)

    time2 = time.time()
    print(time2-time1)
    return cta_data, np.mean(all_dices)      

def cpr2cta_map_direct(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names, cpr_centerlines):
    time1 = time.time()
    ...
    cta_data = np.zeros(cta_shape[::-1], dtype='uint8')
    all_dices = []
    length = int(len(mask_names)/60)
    print(length)
    for cout in range(length):
        c = 0
        cta_data_copy = np.zeros(cta_shape[::-1], dtype='uint8')
        #cta_data_list = []
        for i in range(cpr_counts*60, (cpr_counts+1)*60):
            #cta_data = np.zeros(cta_shape[::-1], dtype='uint8')
            print(mask_names[i])
            cpr_mask = cpr_masks[i]
            #cpr_contour = contour_files[i]
            ...
            for r in range(len(area_index)):
                ...
                for cpr in mask_index:
                    x, y, z = cpr_axis_coord[cpr[0],cpr[1]]
                    #cta_data[int(x), int(y), int(z)] = cpr_mask[cpr[0],cpr[1]]/255
                    #cta_data_copy[int(x), int(y), int(z)] = cpr_mask[cpr[0],cpr[1]]/255
                    cta_data[int(round(x)), int(round(y)), int(round(z))] = cpr_mask[cpr[0],cpr[1]]/255
                    cta_data_copy[int(round(x)), int(round(y)), int(round(z))] = cpr_mask[cpr[0],cpr[1]]/255
            print(len(cta_data_copy[cta_data_copy==1]))
            #cta_data_copy = cta_data.copy()
            #cta_data_list.append(cta_data_copy)
            #if i%20 == 4:
            #    break
        dices = cta2cpr(cpr_coords, cta_data_copy, cpr_map_index, patient, series, mask_names, cpr_counts)
        all_dices.append(dices)
        #print('cpr_counts',cpr_counts)
        '''
        for p in range(cpr_counts*20, (cpr_counts+1)*20):
            #p = i 
            cpr_axis_coord = cpr_coords[p]
            xs, ys, zs = cpr_axis_coord[:, :, 0], cpr_axis_coord[:, :, 1], cpr_axis_coord[:, :, 2]
            cpr_data = ndimage.map_coordinates(cta_data, [xs, ys, zs], order=1, cval=0).astype(cta_data.dtype)
...
def vessel_plaque(patient, series, cta_data):
    vessel_dir = glob.glob(os.path.join(mapping_gen_path, patient, '*', series, 'mask_source/mask_vessel.nii.gz'))[0]
    vessel_data = sitk.ReadImage(vessel_dir)
    vessel_data = sitk.GetArrayFromImage(vessel_data)  
    vessel_data = np.transpose(vessel_data, (2, 1, 0))
    print(vessel_data.shape)
    print(cta_data.shape)
    vessel_data[cta_data==1] = 0 
    vessel_left_data = vessel_data
    save_nii(os.path.join(plaque_mask_newmap_path, patient, series), True, vessel_left_data, 'mask_vessel-plaque_round60.nii.gz')
...
name_list = ['1073318']#['1036623', '1073309', '1073332', '1073330', '1073318', '1036617']#['1036604_60_0416']#['1036602', '1036604', '1036607', '1036631', '1036632', '1036610', '1036619', '1036630', '1036615', '1036625', '1073332', '1036624'] #'1036607', 
all_dices = []

for patient in sorted(os.listdir(plaque_mask_gen_path)):
    if patient not in name_list: #!= '1036602': 
        continue
    print(patient)
    for series in os.listdir(os.path.join(plaque_mask_gen_path, patient)):
        if series == '5F653FE9_CTA':
            continue
        print(series)
        ...
        mask_paths = sorted(glob.glob(os.path.join(plaque_mask_gen_path, patient, series, '*.png')), key=natural_sort_key)
        ...
        #print(mask_names)
        '''
        contour_paths = sorted(glob.glob(os.path.join(plaque_contour_gen_path, patient, series, '*.png')), key=natural_sort_key)
        ...
        cpr_centerline_paths = sorted(glob.glob(os.path.join(cpr_centerline_paths, '*.2d_cpr')), key=natural_sort_key)
        ...
        vessel_mask_paths = sorted(glob.glob(os.path.join(vessel_mask_paths, '*.bmp')), key=natural_sort_key)
        ...
        mapping_paths = sorted(glob.glob(os.path.join(mapping_paths, '*.dat')), key=natural_sort_key)
        ...
        #print(mask_names)
        #print(mapping_names)
        #cta_data, dice = cpr2cta_map(mapping_files, mask_files, cta_shape, roi_index_list, patient, series, mask_names, contour_files, cpr_centerlines)
        ...
        cta_data, dice = cpr2cta_map_direct(mapping_files, mask_files, cta_shape, roi_index_list, patient, series, mask_names, cpr_centerlines)
        all_dices.append(dice)
        save_nii(os.path.join(plaque_mask_newmap_path, patient, series), cta_series.flip, cta_data, 'mask_plaque_round60.nii.gz')

        vessel_plaque(patient, series, cta_data)

print('final_dice', all_dices, np.mean(all_dices))