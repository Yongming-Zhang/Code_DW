...
def divide_plaque(cpr_centerlines, cpr_counts, cpr_masks, contour_files, cpr_coords, all_cpr_coord, all_cpr_contours, all_cta_coord, all_centerlines, all_ids):
    c = 0
    for i in range(cpr_counts*20, (cpr_counts+1)*20):
        ...
        if i%20 == 0 or len(all_cta_coord) == 0:
            print('lailema')
    ...
    print('c1', c)
    return all_cpr_coord, all_cpr_contours, all_cta_coord, all_centerlines, all_ids

def divide_region(all_cpr_coord, cpr_counts, all_ids, cpr_coords, all_cpr_contours, all_centerlines, cta_data):
    ...
    for i in range(len(all_cpr_coord)):
        ...
        for j in range(len(all_cpr_coord[i])):
            id_left, all_axis_coord_left, centerline_left, id_right, all_axis_coord_right, centerline_right, domain_left_counts, domain_right_counts = divide_connectivity_domain(all_cpr_coord, all_ids, cpr_coords, cpr_counts, all_cpr_contours, all_centerlines, i, j, id_left, all_axis_coord_left, centerline_left, id_right, all_axis_coord_right, centerline_right, domain_left_counts, domain_right_counts)
            ...
            while left_i < len(id_left):
                if left_i < len(id_left)-left_counts[id_left[-1]]:  
                    #判断如果有两个相同编号的斑块连通域，则判断这两个的下一个，若有下一个挨着的编号，则分别与下一个最近的连接，
...
def divide_connectivity_domain(all_cpr_coord, all_ids, cpr_coords, cpr_counts, all_cpr_contours, all_centerlines, i, j, id_left, all_axis_coord_left, centerline_left, id_right, all_axis_coord_right, centerline_right, domain_left_counts, domain_right_counts):
    ...
    return id_left, all_axis_coord_left, centerline_left, id_right, all_axis_coord_right, centerline_right, domain_left_counts, domain_right_counts
...
def cpr2cta_map(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names, contour_files, cpr_centerlines):
    ...
    for cout in range(len(cpr_centerlines)):
        ...
        all_cpr_coord, all_cpr_contours, all_cta_coord, all_centerlines, all_ids = divide_plaque(cpr_centerlines, cpr_counts, cpr_masks, contour_files, cpr_coords, all_cpr_coord, all_cpr_contours, all_cta_coord, all_centerlines, all_ids)
        
        divide_region(all_cpr_coord, cpr_counts, all_ids, cpr_coords, all_cpr_contours, all_centerlines, cta_data)
...
name_list = ['1036631', '1036632', '1036610', '1036619', '1036630', '1036615', '1036625', '1073332', '1036624'] #'1036607', 
