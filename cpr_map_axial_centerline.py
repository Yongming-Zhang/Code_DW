...
def cpr2cta_map(cpr_coords, cpr_mask_data, cta_shape, cpr_map_index, patient, series, mask_names, contour_files, cpr_centerlines):
    ...
    for cpr_centerline in cpr_centerlines:
        ...
        all_axis_coord = [[] for s in range(len(all_cpr_coord))]
        for i in range(len(all_cpr_coord)):
            print('len(all_cpr_coord[i])', len(all_cpr_coord[i]))
            all_axis_coord[i] = [[] for s in range(len(all_cpr_coord[i]))]
            for j in range(len(all_cpr_coord[i])):
                ...
                if len(area_cpr_data_index) == 2:
                    print('中心线使该区域分为两个连通域')
                else:
                    print('中心线没有使该区域分为两个连通域', len(area_cpr_data_index))

                all_axis_coord[i][j] = [[] for s in range(2)]
                for k in range(len(area_cpr_data_index)):
                    ...
                    for index in area_cpr:
                        if [index[0], index[1]] in cpr_contour_index:
                            if all_axis_coord[i][j][0] = []:
                                print('该点在中心线左边')
                                all_axis_coord[i][j][0].append([cpr_axis_coord[index[0], index[1]]])
                            if all_axis_coord[i][j][1] == []:
                                print('该点在中心线右边')
                                all_axis_coord[i][j][1].append([cpr_axis_coord[index[0], index[1]]])

     
        print(len(all_axis_coord))
        all_axis_coord_left = []
        all_axis_coord_right = []
        for i in range(len(all_axis_coord)):
            axis_coord_left = []
            axis_coord_right = []
            for j in range(len(all_axis_coord[i])):
                if len(all_axis_coord[i][j][0]) != 0:
                    axis_coord_left.append(all_axis_coord[i][j][0])
                if len(all_axis_coord[i][j][1]) != 0:
                    axis_coord_right.append(all_axis_coord[i][j][1])
            if len(axis_coord_left) != 0:
                all_axis_coord_left.append(axis_coord_left)
            if len(axis_coord_right) != 0:
                all_axis_coord_right.append(axis_coord_right)

        for i in range(len(all_axis_coord_left)):
            for j in range(len(all_axis_coord_left[i])):
                if len(all_axis_coord_left[i]) != 1:
                    if j != len(all_axis_coord_left[i])-1:
                        a_len = len(all_axis_coord_left[i][j])
                        b_len = len(all_axis_coord_left[i][j+1])
                        print('left', a_len, b_len)
                        generate_point_lines(all_axis_coord_left[i][j], all_axis_coord_left[i][j+1], cta_data)
                    elif j == len(all_axis_coord_left[i])-1:
                        a_len = len(all_axis_coord_left[i][j])
                        b_len = len(all_axis_coord_left[i][0]) 
                        print('leftend', a_len, b_len)
                        generate_point_lines(all_axis_coord_left[i][j], all_axis_coord_left[i][0], cta_data)

        for i in range(len(all_axis_coord_right)):
            for j in range(len(all_axis_coord_right[i])):
                if len(all_axis_coord_right[i]) != 1:
                    if j != len(all_axis_coord_right[i])-1:
                        a_len = len(all_axis_coord_right[i][j])
                        b_len = len(all_axis_coord_right[i][j+1]) 
                        print('right', a_len, b_len)
                        generate_point_lines(all_axis_coord_right[i][j], all_axis_coord_right[i][j+1], cta_data)
                    elif j == len(all_axis_coord_right[i])-1:
                        a_len = len(all_axis_coord_right[i][j])
                        b_len = len(all_axis_coord_right[i][0]) 
                        print('rightend', a_len, b_len)
                        generate_point_lines(all_axis_coord_right[i][j], all_axis_coord_right[i][0], cta_data)
...
def generate_lines(a, b, cta_data):
    fz, fy, fx, sz, sy, sx = int(round(a[0][0])), int(round(a[0][1])), int(round(a[0][2])), int(round(b[0][0])), int(round(b[0][1])), int(round(b[0][2]))
